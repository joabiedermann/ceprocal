/*! -----------------------------------------------------------------------------------
    ...
    Template URI: http://admin.pixelstrap.com/Riho/theme
...
(function ($) {
  ...
  function initializeShowMoreButtons() {
    ...
    showMoreBtns.forEach(showMoreBtn => {
      ...
      // Match the nearest valid parent section
      const mainPage = showMoreBtn.closest('.main-page, .application-sec, .custom-feature-btn, .custom-frameworks');
      if (!mainPage) {
        console.error('No valid parent section found for the show-more button.');
        return;
      }
  
      // Default settings
      let initialVisible = 6;
      let itemSelector = '.row > div';
      let displayStyle = 'block';
  
      // Customize based on section type
      if (mainPage.classList.contains('application-sec')) {
        initialVisible = 12;
      } else if (mainPage.classList.contains('custom-feature-btn')) {
        initialVisible = 8;
      } else if (mainPage.classList.contains('custom-frameworks')) {
        initialVisible = 15;
        itemSelector = 'ul > li';
        displayStyle = 'inline-block';
      }
  
      function updateBoxes(showAll = false) {
        const items = mainPage.querySelectorAll(itemSelector);
  
        items.forEach((item, index) => {
          item.style.display = (showAll || index < initialVisible) ? displayStyle : 'none';
        });
  
        if (items.length > initialVisible) {
          showMoreBtn.style.display = 'flex';
          showMoreBtn.innerHTML = showAll
            ? '<span>Show Less <i class="fa fa-chevron-up ms-1"></i></span>'
            : '<span>Show More <i class="fa fa-chevron-down ms-1"></i></span>';
        } else {
      ...
      showMoreBtn.addEventListener('click', () => {
        ...
        updateBoxes(isExpanded);
      });
  
      // MutationObserver to watch for dynamically added items
      const rowContainer = mainPage.querySelector(itemSelector.split(' > ')[0]); // .row or ul
      if (rowContainer) {
