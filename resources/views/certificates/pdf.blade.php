...
<html lang="sp">
    <head>
        ...
        <style>
            @font-face {
                ...
                src: local('Maven Pro'), local('Maven-Pro'), url('{{ public_path('fonts/MavenProRegular.ttf') }}') format('truetype');
            }
            @page {
                size: A4 landscape;
                margin: 0;
    ...
    <body>
        <div class="background">
            <img class="image-bg" src="{{ asset('storage/certificates/templates/background.png') }}" alt="background" width="100%" height="100%">
        </div>
        <div class="content">
            <p class="student_name">{{ $student->name }}</p>
            <p class="culminate">Por haber culminado el curso</p>
            <p class="course"> {{ $course->name }} ({{ $course->hours }} HS)" </p>
            <p class="activity">Actividad organizada por el Centro Paraguayo de Productividad y Calidad - CEPROCAL y la
                <br>
                Unión Industrial Paraguaya - UIP en fecha {{ Carbon\Carbon::parse($course->end_date)->format('d/m/Y') }}
            </p>
            @if ($course->companies->count() > 0)
                <p class="asociate">REALIZADO EN: </p>
                <div class="asociates-logos">
                    @foreach ($course->companies as $company)
                        <img class="asociate-image" src="{{ asset($company->company_logo) }}" alt="{{ $company->company_name }}">
                    @endforeach
                </div>
            @endif
        ...
        <div class="qr">
            <div class="qr_content">
                ...
                @endphp
                {!! QrCode::size(130)->generate($link_content) !!}
            </div>
            ...
            <div class="qr_verify">
                ...
                @endphp
                {!! QrCode::size(130)->generate($link_verify) !!}
            </div>
            <div class="qr_verify_text">
                <span>VERIFICACIÓN DE AUTENTICIDAD</span>
                {{-- <br>
                <span>code: {{ $student->hash_certificate }}</span> --}}
            </div>
        </div>
    </body>
</html>